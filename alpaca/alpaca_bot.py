...
# Bot configuration
ASSETS = ["DOGE/USD", "BCH/USD", "USDT/USD", "LTC/USD"]
ALLOCATION = 10  # Amount in USD per trade
CHECK_INTERVAL = 1800 # Time between evaluations in seconds, 60×60=3,600 seconds(1 hour 60 min, 1min 60 second )
...
def validate_asset(symbol):
    """Check if the asset is valid and tradable."""
    try:
        request_params = CryptoLatestTradeRequest(symbol_or_symbols=[symbol])
        crypto_data_client.get_crypto_latest_trade(request_params)
        return True
    except Exception:
        logger.warning(f"Asset {symbol} is not valid or tradable.")
        return False
...
def main():
    ...
    valid_assets = [asset for asset in ASSETS if validate_asset(asset)]
