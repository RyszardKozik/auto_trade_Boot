...
CHECK_INTERVAL = 1800  # Time between evaluations in seconds
    ...
    for symbol, data in list(positions.items()):
        try:
            ...
            if not current_price:
                logger.warning(f"Skipping {symbol} due to unavailable price.")
                continue

            qty = data["qty"]
            logger.info(f"Selling {qty} of {symbol} at {current_price}.")
...
if __name__ == "__main__":
    # Add a way to close all positions at the user's request
    action = input("Enter 'run' to start bot or 'close' to close all positions: ").strip().lower()
    if action == "close":
        close_all_positions()
    elif action == "run":
        main()
