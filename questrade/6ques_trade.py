...
API_BASE_URL = os.getenv("API_BASE_URL", "https://api01.iq.questrade.com")
TIMEZONE = timezone("US/Eastern")

# Validate required environment variables
if not ACCESS_TOKEN:
    logging.error("Missing Questrade API access token. Please check your .env file.")
    raise ValueError("Missing required environment variable: ACCESS_TOKEN.")
...
TRADE_AMOUNT = 10  # $10 per trade for each asset
...
def send_request(endpoint, method="GET", params=None, data=None):
    ...
    try:
        ...
        if response.status_code == 401:  # Handle expired token
            logging.error("Access token expired. Please refresh your token manually.")
