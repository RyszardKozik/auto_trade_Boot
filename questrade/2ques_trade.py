...
def exchange_code_for_token(code):
    ...
    try:
        ...
        if not ACCESS_TOKEN or not API_SERVER:
            raise ValueError("Invalid response: Missing 'access_token' or 'api_server'")
        logging.info("Token obtained successfully.")
    except Exception as e:
        log_error(f"Failed to exchange code for token: {e}. Response: {response.text if 'response' in locals() else 'No response'}")
        raise
...
def refresh_token():
    ...
    try:
        ...
        log_error(f"Failed to refresh token: {e}. Response: {response.text if 'response' in locals() else 'No response'}")
        raise
...
def make_questrade_request(endpoint, method="GET", payload=None):
    ...
    try:
        ...
        log_error(f"API request error at {endpoint}: {e}. Response: {response.text if 'response' in locals() else 'No response'}")
        raise
